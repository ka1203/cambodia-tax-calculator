...
// ── NAV items (sidebar only – Home / Calculate / About) ──────────────────────
const NAV_ITEMS = [
  {
    id: "home",
    label: "Home",
...
// ── Tax category cards – each has a `page` key matching your app's setPage ───
const SERVICES = [
  ...
  { page: "prothab_tax",  emoji: "🏭", title: "ពន្ធប្រថាប់ត្រា",             desc: "គណនា ពន្ធដែលត្រូវបង់នៅពេលមានការទិញ-លក់ ដោះដូរ ឬធ្វើអំណោយ ",  color: "#F5F3FF", accent: "#7C3AED" } ,
...
// ════════════════════════════════════════════════════════════════════════════════
// ROOT – drop-in replacement for your HomePage({ setPage })
// ════════════════════════════════════════════════════════════════════════════════
export default function HomePage({ setPage }) {
  // activeNav drives which SIDEBAR tab is highlighted (home / calculate / about)
  const [activeNav, setActiveNav]   = useState("home");
  const [sidebarOpen, setSidebarOpen] = useState(false);

  // navigate: update sidebar highlight + call parent setPage if provided
  const go = (pageKey) => {
    // map sidebar tabs to what the parent router expects
    const sidebarToParent = { home: "home", calculate: "home", about: "home" };
    // For sidebar tabs we stay on this component; for tax cards we delegate
    const isSidebarTab = NAV_ITEMS.some((n) => n.id === pageKey);
    if (isSidebarTab) {
      setActiveNav(pageKey);
    } else {
      // tax card clicked – hand off to the real router
  ...
  return (
    <div style={{ fontFamily: "'Battambang',serif", background: "#F0F4FF", minHeight: "100vh", display: "flex" }}>
      ...
      <Sidebar activeNav={activeNav} go={go} setPage={setPage} className="sidebar-desktop"
        style={{ position:"fixed", top:0, left:0, height:"100vh", width:240, zIndex:200 }}
      />

      {/* Sidebar – mobile (slide-in) */}
      {sidebarOpen && (
        <Sidebar activeNav={activeNav} go={go} setPage={setPage} className="sidebar-mobile"
      ...
      <main className="main-content" style={{ marginLeft:240, flex:1, minWidth:0, overflowX:"hidden" }}>
        {activeNav === "home"      && <HomeContent      go={go} setPage={setPage} />}
        {activeNav === "calculate" && <CalculateContent go={go} />}
...
// ── Sidebar ──────────────────────────────────────────────────────────────────
function Sidebar({ activeNav, go, setPage, className, style }) {
  return (
    <aside className={className} style={{
      ...
      <div style={{ background:"#F0F4FF", borderRadius:12, padding:14, textAlign:"center" }}>
        ...
        <div style={{ fontSize:10, color:"#9CA3AF", fontFamily:"'Battambang',serif" }}>Free · Accurate · Fast</div>
      </div>
    </aside>
  );
}
...
// ── Home content ─────────────────────────────────────────────────────────────
function HomeContent({ go, setPage }) {
  return (
    <div style={{ background:"#F0F4FF", minHeight:"100vh" }}>

      {/* HERO */}
      <section style={{ position:"relative", overflow:"hidden", background:"linear-gradient(135deg,#EFF6FF 0%,#DBEAFE 60%,#E0F2FE 100%)", padding:"64px 40px 52px" }}>
        <div className="hblob" style={{ width:360, height:360, background:"#BFDBFE", top:-80, right:-60 }} />
        <div className="hblob" style={{ width:260, height:260, background:"#BAE6FD", bottom:-50, left:-40 }} />

        <div style={{ maxWidth:1080, margin:"0 auto", display:"grid", gridTemplateColumns:"1fr 1fr", gap:40, alignItems:"center", position:"relative", zIndex:2 }}>
          {/* Left */}
          <div className="slide-left">
            ...
            <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
              <button className="btn-p" onClick={() => go("calculate")}>ចាប់ផ្ដើមគណនា</button>
              <button className="btn-o" onClick={() => go("about")}>សិក្សាបន្ថែម</button>
            </div>
          ...
          <div className="slide-right float-anim" style={{ display:"flex", justifyContent:"center" }}>
            <div style={{ position:"relative", width:300, height:300 }}>
              ...
              <div style={{ position:"absolute", top:"40%", right:-10, background:"#1D4ED8", color:"#fff", borderRadius:10, padding:"7px 12px", boxShadow:"0 6px 18px rgba(29,78,216,0.3)", fontFamily:"'Battambang',serif", fontSize:12, fontWeight:700, whiteSpace:"nowrap" }}>📋 ម៉ូឌុល ៤+</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <svg viewBox="0 0 1440 48" preserveAspectRatio="none" style={{ display:"block", width:"100%", height:48 }}>
        <path d="M0,32 C360,0 1080,48 1440,16 L1440,0 L0,0 Z" fill="#DBEAFE"/>
      </svg>
      ...
      {/* Footer CTA */}
      <section style={{ background:"linear-gradient(135deg,#1D4ED8,#1e40af)", padding:"52px 40px", textAlign:"center" }}>
        <div style={{ maxWidth:560, margin:"0 auto" }}>
          <div style={{ fontSize:38, marginBottom:14 }}>🇰🇭</div>
          <h2 style={{ color:"#fff", fontSize:26, fontWeight:700, fontFamily:"'Battambang',serif", marginBottom:12 }}>ចាប់ផ្ដើមគណនាពន្ធថ្ងៃនេះ</h2>
          <p style={{ color:"#BFDBFE", fontSize:14, lineHeight:2, fontFamily:"'Battambang',serif", marginBottom:24 }}>
            ប្រព័ន្ធពន្ធកម្ពុជា ២០២៦ — ឥតគិតថ្លៃ ត្រឹមត្រូវ ងាយស្រួលប្រើ
          </p>
          <button className="btn-p" style={{ background:"#fff", color:"#1D4ED8", animation:"none" }} onClick={() => go("calculate")}>
            ចាប់ផ្ដើមឥឡូវ →
          </button>
        </div>
      </section>
...
// ── Calculate content (shows the same cards grid) ────────────────────────────
function CalculateContent({ go }) {
  return (
    <div style={{ padding:"52px 40px", maxWidth:1080, margin:"0 auto", minHeight:"100vh" }}>
      <div className="fade-up-1">
        ...
        <h1 style={{ fontSize:32, fontWeight:700, color:"#0F172A", fontFamily:"'Battambang',serif", marginBottom:10 }}>ជ្រើសរើសប្រភេទពន្ធ</h1>
        <p style={{ color:"#6B7280", fontSize:14, lineHeight:2, fontFamily:"'Battambang',serif", marginBottom:36 }}>
          ជ្រើសរើសកម្មវិធីខាងក្រោម ដើម្បីចាប់ផ្ដើមគណនាពន្ធ
        </p>
      </div>
      <ServicesGrid setPage={null} go={go} />
    </div>
  );
}

// ── Shared services grid ──────────────────────────────────────────────────────
// When setPage is provided, card clicks call setPage (real navigation).
// When only go is provided (Calculate tab), we still use setPage from closure if available.
function ServicesGrid({ setPage, go }) {
  ...
  return (
    <style>{`
      ...
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .btn-o {
        background:transparent; color:#1D4ED8; border:2px solid #1D4ED8;
        padding:11px 28px; border-radius:50px; cursor:pointer;
        font-family:'Battambang',serif; font-weight:700; font-size:15px;
        transition:background .2s, color .2s, transform .15s;
      }
      .btn-o:hover { background:#1D4ED8; color:#fff; transform:scale(1.04); }
      ...
      @media (max-width:768px) {
        ...
        .main-content   { margin-left:0 !important; padding-top:68px !important; }
      }
    `}</style>
  );
}