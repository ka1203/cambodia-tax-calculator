...
// ── SPECIAL TAX RATES ─────────────────────────────────────────
const GOODS_RATES = [
  { id: "wine",       label: "ស្រា (Wine/Spirits) — គ្រប់ប្រភេទ",       rate: 0.35, note: "×90% Invoice" },
  { id: "beer",       label: "ស្រាបៀរ — គ្រប់ប្រភេទ",                   rate: 0.30, note: "×90% Invoice" },
  { id: "cigar",      label: "ស៊ីហ្គា — គ្រប់ប្រភេទ",                   rate: 0.25, note: "×90% Invoice" },
  { id: "cigarette",  label: "បារី — គ្រប់ប្រភេទ",                      rate: 0.20, note: "×90% Invoice" },
  { id: "energy",     label: "ភេស្ជៈប៉ូវកម្លាំង (Energy Drink)",        rate: 0.15, note: "×90% Invoice" },
  { id: "softdrink",  label: "ភេស្ជៈគ្មានជាតិសុរា (Soft Drink)",        rate: 0.10, note: "×90% Invoice" },
  { id: "juice",      label: "ទឹកផ្លែឈើ (Fruit Juice)",                 rate: 0.05, note: "×90% Invoice" },
  { id: "plastic",    label: "ផលិតផលប្លាស្ទិក",                          rate: 0.10, note: "×90% Invoice" },
  { id: "cement",     label: "ស៊ីម៉ងត៍ — គ្រប់ប្រភេទ",                  rate: 0.05, note: "×90% Invoice" },
  { id: "electronic", label: "គ្រឿងអេឡិចត្រូនិក",                       rate: null, note: "អត្រាកំណត់ដាច់ដោយឡែក" },
  { id: "moto",       label: "ទោចក្រយានយន្ត",                           rate: null, note: "អត្រាកំណត់ដាច់ដោយឡែក" },
  { id: "car",        label: "រថយន្ត",                                     rate: null, note: "អត្រាកំណត់ដាច់ដោយឡែក" },
  { id: "sparepart",  label: "គ្រឿងបន្លាស់យានយន្ត",                      rate: null, note: "អត្រាកំណត់ដាច់ដោយឡែក" },
  { id: "cosmetic",   label: "ផលិតផលគ្រឿងសម្អាង (Cosmetics)",           rate: null, note: "អត្រាកំណត់ដាច់ដោយឡែក" },
  { id: "glass",      label: "ផលិតផលកញ្ចក់ — គ្រឿងសំណង់",              rate: null, note: "អត្រាកំណត់ដាច់ដោយឡែក" },
];

const SERVICE_RATES = [
  { id: "telecom",   label: "សេវាទូរគមនាគមន៍ (Telecom)",         rate: 0.03 },
  { id: "airline",   label: "សេវាដឹកជញ្ជូន អ្នកដំណើរ (Airline)", rate: 0.10 },
  { id: "entertain", label: "សេវាលំហែកម្សាន្ត (Entertainment)",   rate: 0.10 },
];
...
const S = {
  ...
  toggleGroup: { display: "flex", gap: 0, borderRadius: 12, overflow: "hidden", border: "1px solid #CBD5E1", marginBottom: 16 },
  toggleBtn: (on) => ({
    flex: 1, padding: "10px 14px", border: "none", cursor: "pointer", fontSize: 13,
    fontWeight: on ? 700 : 500, fontFamily: FONT,
    background: on ? "linear-gradient(135deg,#2563EB,#1D4ED8)" : "#FFFFFF",
...
// ── RESULT BLOCK ──────────────────────────────────────────────
function ResultBlock({ invoiceAmt, taxBase, taxAmt, taxRate, baseType }) {
  return (
    <>
      <div style={S.metricGrid}>
        <div style={S.metric}>
          <div style={S.mLabel}>ចំនួនទឹកប្រាក់ Invoice</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#0B1F4E", fontFamily: FONT }}>{fmtKHR(invoiceAmt)}</div>
        </div>
        <div style={S.metric}>
          <div style={S.mLabel}>មូលដ្ឋានគិតអាករ</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#2563EB", fontFamily: FONT }}>{fmtKHR(taxBase)}</div>
        </div>
        <div style={{ ...S.metric, border: "2px solid #FEE2E2", background: "#FEF2F2" }}>
          <div style={S.mLabel}>អាករពិសេស ({(taxRate * 100).toFixed(0)}%)</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#c0392b", fontFamily: FONT }}>{fmtKHR(taxAmt)}</div>
        </div>
      </div>
      <div style={S.card}>
        ...
        <table style={S.tbl}>
          ...
          <tbody>
            <tr>
              ...
              <td style={S.td}>—</td>
              <td style={{ ...S.td, color: "#2563EB", fontWeight: 700 }}>{fmtKHR(invoiceAmt)}</td>
            </tr>
            <tr>
              <td style={S.td}>
                {baseType === "domestic" ? "មូលដ្ឋានគិតអាករ (× 90%)"
                  : baseType === "import" ? "មូលដ្ឋានគិតអាករ (CIF + ពន្ធនាំចូល)"
                  : "មូលដ្ឋានគិតអាករ (ថ្លៃសេវា)"}
              </td>
              <td style={S.td}>
                {baseType === "domestic" ? `${fmtKHR(invoiceAmt)} × 90%`
                  : baseType === "import" ? "CIF + Import Duty"
                  : fmtKHR(invoiceAmt)}
              </td>
              <td style={{ ...S.td, color: "#2563EB", fontWeight: 700 }}>{fmtKHR(taxBase)}</td>
            </tr>
            <tr>
              <td style={S.td}>អាករពិសេស</td>
              <td style={S.td}>{(taxRate * 100).toFixed(0)}%</td>
              <td style={{ ...S.td, color: "#DC2626", fontWeight: 700 }}>{fmtKHR(taxAmt)}</td>
            </tr>
          </tbody>
        </table>
        <div style={S.dedTotal}>
          <span>អាករពិសេសត្រូវបង់ ({fmtKHR(taxBase)} × {(taxRate * 100).toFixed(0)}%)</span>
          <span>{fmtKHR(taxAmt)}</span>
        </div>
        <div style={S.note}>
          <strong>រូបមន្ត:</strong><br />
          {baseType === "domestic" && <>• មូលដ្ឋាន = {fmtKHR(invoiceAmt)} × 90% = {fmtKHR(taxBase)}<br /></>}
          {baseType === "import"   && <>• មូលដ្ឋាន = CIF + ពន្ធនាំចូល = {fmtKHR(taxBase)}<br /></>}
          {baseType === "service"  && <>• មូលដ្ឋាន = ថ្លៃសេវា = {fmtKHR(taxBase)}<br /></>}
          • អាករ = {fmtKHR(taxBase)} × {(taxRate * 100).toFixed(0)}% = <strong>{fmtKHR(taxAmt)}</strong><br /><br />
...
// ══════════════════════════════════════════════════════════════
// TAB 1 — DOMESTIC GOODS  (incl / excl toggle)
// ══════════════════════════════════════════════════════════════
function DomesticTab() {
  const [goodsType, setGoodsType] = useState("beer");
  const [amount,    setAmount]    = useState("");
  const [vatMode,   setVatMode]   = useState("excl"); // "excl" | "incl"
  ...
  function calculate() {
    ...
    if (!gr.rate) { setResult({ noRate: true, label: gr.label }); return; }

    // If user gave Invoice incl. VAT (10%), extract base first
    const invoice = vatMode === "incl" ? amt / 1.10 : amt;
    const taxBase = invoice * 0.90;
    const taxAmt  = taxBase * gr.rate;
    setResult({ invoiceAmt: invoice, taxBase, taxAmt, taxRate: gr.rate, vatMode, origAmt: amt });
  }
  ...
  return (
    <>
      <div style={S.card}>
          ...
          <select style={S.select} value={goodsType} onChange={e => { setGoodsType(e.target.value); setResult(null); }}>
            {GOODS_RATES.map(r => (
              <option key={r.id} value={r.id}>
                {r.label} {r.rate ? `(${(r.rate * 100).toFixed(0)}%)` : "(អត្រាផ្ទាល់)"}
              </option>
            ))}
          </select>
        </div>
        ...
        {/* incl / excl toggle */}
        <div style={S.field}>
          <label style={S.label}>Invoice ដែលអ្នកដឹង</label>
          <div style={S.toggleGroup}>
            <button style={S.toggleBtn(vatMode === "excl")}
              onClick={() => { setVatMode("excl"); setResult(null); }}>
              excl. VAT &amp; excl. ET
            </button>
            <button style={S.toggleBtn(vatMode === "incl")}
              onClick={() => { setVatMode("incl"); setResult(null); }}>
              incl. VAT (10%) excl. ET
            </button>
          </div>
        </div>
        ...
        <div style={S.field}>
          <label style={S.label}>
            តម្លៃ Invoice (៛) — {vatMode === "excl" ? "excl. VAT · excl. ET" : "incl. VAT (10%) · excl. ET"}
          </label>
          <input style={S.input} type="number" placeholder="ឧ: 40,000,000"
            value={amount} onChange={e => { setAmount(e.target.value); setResult(null); }} />
        </div>

        {vatMode === "excl" ? (
          <div style={S.note}>
            • Invoice excl. VAT &amp; excl. ET → មូលដ្ឋាន = Invoice × 90%<br />
            • អាករ = មូលដ្ឋាន × <strong>{selectedRate?.rate ? (selectedRate.rate * 100).toFixed(0) : "—"}%</strong>
          </div>
        ) : (
          <div style={{ ...S.note, background: "#FFFBEB", border: "1px solid #FDE68A", color: "#92400E" }}>
            • Invoice incl. VAT → Invoice (excl.) = Input ÷ 1.10 → មូលដ្ឋាន = Invoice (excl.) × 90%<br />
            • អាករ = មូលដ្ឋាន × <strong>{selectedRate?.rate ? (selectedRate.rate * 100).toFixed(0) : "—"}%</strong>
          </div>
        )}
      </div>
        ...
        <>
          {result.vatMode === "incl" && (
            <div style={{ ...S.note, marginBottom: 16 }}>
              ជំហានទី 1: Invoice (incl. VAT) {fmtKHR(result.origAmt)} ÷ 1.10 = Invoice (excl.) <strong>{fmtKHR(result.invoiceAmt)}</strong>
            </div>
          )}
          <ResultBlock {...result} baseType="domestic" />
        </>
      ) : null}
    </>
  );
...
function ImportTab() {
  ...
  function calculate() {
    ...
    const taxAmt  = taxBase * gr.rate;
    setResult({ invoiceAmt: cif, taxBase, taxAmt, taxRate: gr.rate, duty });
  }
  ...
  return (
    <>
      <div style={S.card}>
        ...
        <div style={S.row2}>
          <div style={S.field}>
            <label style={S.label}>តម្លៃ CIF (៛)</label>
            <input style={S.input} type="number" placeholder="ឧ: 20,000,000"
              value={cifAmount} onChange={e => { setCifAmount(e.target.value); setResult(null); }} />
          </div>
          <div style={S.field}>
            <label style={S.label}>ពន្ធនាំចូល (៛)</label>
            <input style={S.input} type="number" placeholder="ឧ: 2,000,000"
        ...
        <div style={S.note}>
          ...
          {selectedRate?.rate
            ? <>• អាករ = (CIF + ពន្ធនាំចូល) × <strong>{(selectedRate.rate * 100).toFixed(0)}%</strong></>
            : <strong style={{ color: "#92400E" }}>⚠ អត្រាកំណត់ដាច់ដោយឡែក — សូមពិនិត្យ Prakas GDT</strong>
          }<br />
          • CIF = Cost + Insurance + Freight
        </div>
      </div>
        ...
        <>
          <div style={S.metricGrid}>
            <div style={S.metric}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: "#0B1F4E", fontFamily: FONT }}>{fmtKHR(result.invoiceAmt)}</div>
            </div>
            <div style={S.metric}>
              <div style={S.mLabel}>ពន្ធនាំចូល</div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#f97316", fontFamily: FONT }}>{fmtKHR(result.duty)}</div>
            </div>
            <div style={S.metric}>
              <div style={S.mLabel}>មូលដ្ឋានគិតអាករ</div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#2563EB", fontFamily: FONT }}>{fmtKHR(result.taxBase)}</div>
            </div>
            <div style={{ ...S.metric, border: "2px solid #FEE2E2", background: "#FEF2F2" }}>
              <div style={S.mLabel}>អាករពិសេស ({(result.taxRate * 100).toFixed(0)}%)</div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#c0392b", fontFamily: FONT }}>{fmtKHR(result.taxAmt)}</div>
            </div>
          </div>
          <div style={S.card}>
            <div style={S.cardTitle}>តារាងលម្អិត</div>
            <table style={S.tbl}>
              ...
              <tbody>
                <tr>
                  <td style={S.td}>CIF (Cost + Insurance + Freight)</td>
                  <td style={S.td}>—</td>
                  <td style={{ ...S.td, color: "#2563EB", fontWeight: 700 }}>{fmtKHR(result.invoiceAmt)}</td>
                </tr>
                <tr>
                  <td style={S.td}>+ ពន្ធនាំចូល (Import Duty)</td>
                  <td style={S.td}>—</td>
                  <td style={{ ...S.td, color: "#f97316", fontWeight: 700 }}>{fmtKHR(result.duty)}</td>
                </tr>
                <tr>
                  <td style={{ ...S.td, fontWeight: 700 }}>= មូលដ្ឋានគិតអាករ</td>
                  <td style={S.td}>—</td>
                  <td style={{ ...S.td, color: "#2563EB", fontWeight: 700 }}>{fmtKHR(result.taxBase)}</td>
                </tr>
                <tr>
                  <td style={S.td}>អាករពិសេស</td>
                  <td style={S.td}>{(result.taxRate * 100).toFixed(0)}%</td>
                  <td style={{ ...S.td, color: "#DC2626", fontWeight: 700 }}>{fmtKHR(result.taxAmt)}</td>
                </tr>
              </tbody>
            </table>
            <div style={S.dedTotal}>
              <span>អាករពិសេសត្រូវបង់ × {(result.taxRate * 100).toFixed(0)}%</span>
              <span>{fmtKHR(result.taxAmt)}</span>
            </div>
            <div style={S.note}>
              <strong>រូបមន្ត:</strong><br />
              • មូលដ្ឋាន = {fmtKHR(result.invoiceAmt)} + {fmtKHR(result.duty)} = {fmtKHR(result.taxBase)}<br />
              • អាករ = {fmtKHR(result.taxBase)} × {(result.taxRate * 100).toFixed(0)}% = <strong>{fmtKHR(result.taxAmt)}</strong><br /><br />
...
// ══════════════════════════════════════════════════════════════
// TAB 3 — SERVICE  (incl / excl toggle)
// ══════════════════════════════════════════════════════════════
function ServiceTab() {
  ...
  function calculate() {
    ...
    const invoice = vatMode === "incl" ? amt / 1.10 : amt;
    const taxAmt  = invoice * sr.rate;
    setResult({ invoiceAmt: invoice, taxBase: invoice, taxAmt, taxRate: sr.rate, vatMode, origAmt: amt });
  }
    ...
    <>
      <div style={S.card}>
        ...
        <div style={S.field}>
          ...
          <select style={S.select} value={svcType} onChange={e => { setSvcType(e.target.value); setResult(null); }}>
            {SERVICE_RATES.map(r => (
              <option key={r.id} value={r.id}>{r.label} ({(r.rate * 100).toFixed(0)}%)</option>
            ))}
          </select>
        </div>
        ...
        <div style={S.field}>
          <label style={S.label}>
            ថ្លៃសេវា Invoice (៛) — {vatMode === "excl" ? "excl. VAT · excl. ET" : "incl. VAT (10%) · excl. ET"}
          </label>
          <input style={S.input} type="number" placeholder="ឧ: 4,000,000"
            value={amount} onChange={e => { setAmount(e.target.value); setResult(null); }} />
        </div>

        {vatMode === "excl" ? (
          <div style={S.note}>
            • មូលដ្ឋានគិតអាករ = ថ្លៃសេវា Invoice (គ្មាន × 90%)<br />
            • អាករ = ថ្លៃសេវា × <strong>{selectedRate ? (selectedRate.rate * 100).toFixed(0) : "—"}%</strong>
          </div>
        ) : (
          <div style={{ ...S.note, background: "#FFFBEB", border: "1px solid #FDE68A", color: "#92400E" }}>
            • Invoice (excl.) = Input ÷ 1.10 → អាករ = Invoice (excl.) × <strong>{selectedRate ? (selectedRate.rate * 100).toFixed(0) : "—"}%</strong>
          </div>
        )}
      </div>
      ...
      {result && (
        <>
          ...
          <ResultBlock {...result} baseType="service" />
        </>
      )}
    </>
  );
  ...
  return (
    <div style={S.page}>
      <div style={S.wrap}>
        ...
        <div style={S.header}>
          <div style={S.h1}>អាករពិសេស (Special Tax)</div>
